...
class Author extends AbstractDb
{
    ...
    )
    {
        ...
        $this->authorProductTable  = $this->getTable('ark_infusion_author_product');
        $this->authorCategoryTable = $this->getTable('ark_infusion_author_category');
    ...
    {
        $this->_init('ark_infusion_author', 'author_id');
    }
    ...
    protected function _beforeDelete(AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('ark_infusion_author_store'), $condition);
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                [
                    'ark_infusion_author_store' => $this->getTable('ark_infusion_author_store')
                ],
                $this->getMainTable() . '.author_id = ark_infusion_author_store.author_id',
                []
            )//TODO: check if is_active filter is needed
                ->where('is_active = ?', 1)
                ->where(
                    'ark_infusion_author_store.store_id IN (?)',
                    $storeIds
                )
                ->order('ark_infusion_author_store.store_id DESC')
                ->limit(1);
        }
        return $select;
    }
    ...
    protected function getLoadByUrlKeySelect($urlKey, $store, $isActive = null)
    {
        $select = $this->getConnection()
            ...
            ->join(
                ['author_store' => $this->getTable('ark_infusion_author_store')],
    ...
    public function lookupStoreIds($authorId)
    {
        ...
        $select = $adapter->select()->from(
            $this->getTable('ark_infusion_author_store'),
            'store_id'
        )
            ->where(
                'author_id = ?',
    ...
    protected function saveStoreRelation(AuthorModel $author)
    {
        ...
        $table = $this->getTable('ark_infusion_author_store');
    ...
    protected function saveProductRelation(AuthorModel $author)
    {
        ...
        if (!empty($insert) || !empty($delete)) {
            ...
            $this->eventManager->dispatch(
                'ark_infusion_author_change_products',
    ...
    public function saveAuthorProductRelation(Product $product, $authors)
    {
        ...
        if (!empty($insert) || !empty($delete)) {
            ...
            $this->eventManager->dispatch(
                'ark_infusion_product_change_authors',
    ...
    protected function saveCategoryRelation(AuthorModel $author)
    {
        ...
        if (!empty($insert) || !empty($delete)) {
            ...
            $this->eventManager->dispatch(
                'ark_infusion_author_change_categories',
    ...
    public function saveAuthorCategoryRelation($category, $authors)
    {
        ...
        if (!empty($insert) || !empty($delete)) {
            $authorIds = array_unique(array_merge(array_keys($insert), array_keys($delete)));
            $this->eventManager->dispatch(
                'ark_infusion_category_change_authors',
